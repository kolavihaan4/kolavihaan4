"""
Build a neofetch-style info card SVG to sit to the RIGHT of the ASCII banner:
colored key/value rows for role, focus, and highlights.

Static content, hand-authored below. Lines fade/slide in on a short stagger so
it feels like the panel is printing. STATIC=1 emits the frozen state.
"""
import html
import os
...
ROWS = [
    ("host",),
    ("kv", "Now", "AI Instructor @ KKRGENAI"),
    ("kv", "Focus", "Applied AI + Cybersecurity"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">vihaan@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">vihaan</tspan><tspan fill="{MUTED}">@</tspan>'
